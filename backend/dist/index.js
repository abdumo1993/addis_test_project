...
// setup mongoose
const MONGODB_URI = process.env.MONGODB_URI ?? "mongodb://localhost:27017/addis_software";
...
app.patch("/api/song/:id", async (req, res) => {
    ...
    try {
        ...
        const updatedSong = await SongModel.findByIdAndUpdate(id, { $set: updateData }, { new: true, runValidators: true }).lean();
        if (!updatedSong) {
            return res
                .status(404)
